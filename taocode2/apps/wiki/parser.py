...
class WikiProcessor(object):
    ...
    def __init__(self, formatter, name, args=None):
        ...
        self.processor = builtin_processors.get(name.lower())
        """
    ...
    def _image_processor(self, text):
        link = self.formatter.context.img_resolver(text)
...
class Formatter(object):
    ...
    def _make_link(self, ns, target, match, label, fullmatch):
        ...
        result = self.context.link_resolver(ns, target)#, escape(label, False))
        if result is not None:
            v = label.split(' ', 1)
            if len(v) == 2:
                label = v[1]         
            return tag.a(escape(label, False), href=result[1], 
                         class_ = result[0] and 'wiki' or 'wiki missing') 
    ...
    def _make_ext_link(self, url, text, title=''):
        local_url = self.context.base_url
    ...
    def _parse_heading(self, match, fullmatch, shorten):
        ...
        if htext.endswith(hdepth):
            htext = htext[:-depth]
        heading = format_to_oneliner(self.context, htext, False)
        if anchor:
            anchor = anchor[1:]
        else:
        ...
        if shorten:
            heading = format_to_oneliner(self.context, htext, True)
    ...
    def _definition_formatter(self, match, fullmatch):
        ...
        tmp += '<dt>%s</dt><dd>' % format_to_oneliner(self.context,
                                                      definition)
    ...
    def close_quote_block(self, escape_newlines):
        if self._quote_buffer:
            ...
            Formatter(self.context).format(self._quote_buffer,
...
class InlineHtmlFormatter(object):
    ...
    def __init__(self, context, wikidom):
        ...
        if isinstance(wikidom, basestring):
            wikidom = WikiParser().parse(wikidom)
    ...
    def generate(self, shorten=False):
        ...
        out = StringIO()
        OneLinerFormatter(self.context).format(self.wikidom, out,
                                               shorten)
        return out.getvalue()


def to_html(s, context, escape_newlines=False):
    out = StringIO()
    Formatter(context).format(s, out,
                              escape_newlines)
    return out.getvalue()

def format_to_oneliner(context, wikidom, shorten=None):
    if not wikidom:
        return ''
    if shorten is None:
...
def format_to(flavor, context, wikidom, **options):
    if flavor is None:
        flavor = context.get_hint('wiki_flavor', 'html')
    if flavor == 'oneliner':
        return format_to_oneliner(context, wikidom, **options)
    else:
        return format_to_html(context, wikidom, **options)

def format_to_html( context, wikidom, escape_newlines=None):
    if not wikidom:
        return Markup()
